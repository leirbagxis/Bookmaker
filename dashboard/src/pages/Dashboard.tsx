...
export default function Dashboard() {
  ...
  // Lógica de Scroll para a seção de Inventário
  useEffect(() => {
    if (!loading && profile && location.pathname.endsWith('/inventory')) {
      setTimeout(() => {
  ...
  if (loading) return (
    <div className="flex flex-col items-center justify-center py-xl">
      ...
      <div className="status-label tracking-[0.3em]">Autenticando...</div>
    </div>
  );

  if (!profile) return (
    <div className="refined-card bg-red-50 p-xl text-center border border-red-100">
      <div className="status-label text-red-500">Falha na Conexão</div>
      <div className="text-sm font-bold opacity-60 text-red-900">Não foi possível carregar seu perfil.</div>
    </div>
  );
  ...
  return (
    <div className="space-y-lg animate-fade-in pb-20 font-sans">
      <header className="mb-8 border-b border-border pb-6">
        <h2 className="text-4xl font-black text-black tracking-tighter truncate pr-4 uppercase">
          {getGreeting()}, {profile.first_name}!
        </h2>
        <p className="text-muted font-bold text-[10px] uppercase tracking-[0.2em] mt-2">
          STATUS CENTRAL - <span className="text-black font-black uppercase">{getBotDisplayName()}</span>
        </p>
      </header>
      ...
      {activeTab === 'history' ? (
        ...
        <div className="space-y-lg">
          ...
          <div id="inventory-section" className="scroll-mt-24">
            <Inventory
