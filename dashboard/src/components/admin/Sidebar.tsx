      ...
      <div className="h-full flex flex-col p-6 overflow-y-auto no-scrollbar">
        <div className="mb-10">
          <Link to={`/${botId}`} className="flex items-center gap-3 mb-8 group" onClick={() => setIsSidebarOpen(false)}>
            <div className="w-10 h-10 bg-primary text-black rounded-xl flex items-center justify-center font-black text-xl transition-all duration-500 group-hover:rotate-6 group-hover:scale-110 shadow-lg">B</div>
            <div className="transition-transform duration-500 group-hover:translate-x-1">
              <h1 className="font-black text-base leading-none tracking-tighter normal-case">Bot SaaS</h1>
        ...
        <div className="mt-auto pt-6 border-t border-white/10 space-y-3">
          ...
          <button 
            onClick={loadData} 
            disabled={isLoading} 
            className="bg-white text-black font-black uppercase px-4 h-12 text-[10px] tracking-widest rounded-full transition-all active:scale-95 disabled:opacity-50 flex items-center justify-center gap-2 hover:bg-primary shadow-xl"
          >
            <RefreshCcw size={14} className={cn("transition-all duration-1000", isLoading ? "animate-spin" : "")} />
            Sincronizar
          </button>
        </div>
      </div>
